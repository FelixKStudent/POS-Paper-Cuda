...
TODO Cuda used under the hood for most things
...
image::Images/MatrixBlocks.png[TODO fix picture]
...
image::Images/Flat.png[TODO fix picture]
...
With our previous matrix multiplication example we are reading elements from out input matrices multiple times. For example, as shown in the picture below, both thread(0, 0) and thread(0, 1) will read the first row of our A input matrix from global memory. This is unefficiant and is likely to cause memory bandwidth issues. With tiling we will get our threads to cooperate on this reading, and load parts("tiles") of our data into shared local memory for faster overall access.

.Diagram of the CUDA memory hierarchy, source: https://nichijou.co/cuda7-tiling/
image::Images/tilingload.jpg[TODO fix picture, width = 300]

One factor to look at whether to implement tiling is the ration of floating point operations to global memory loads for a kernel. Taking a look at our original code as shown below.

.Diagram of the CUDA memory hierarchy, source: https://nichijou.co/cuda7-tiling/
image::Images/RatioMatrix.png[TODO fix picture, width = 500]
...
* If you are interested in diving deeper into the memory optimization aspects looking at "Texture Memory", a type of memory found on new Nvidia GPUs might be interesting
* For work on machine learning/neural networks the most intersting thing will be CuDNN, the library that TODO